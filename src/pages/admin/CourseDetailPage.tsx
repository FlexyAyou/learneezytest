                    ...
                    <AccordionItem key={index} value={`module-${index}`} className="border-2 rounded-lg mb-3 overflow-hidden">
                      ...
                      <AccordionContent className="bg-gray-50">
                        <div className="space-y-4 p-4">
                          ...
                          {/* Contenu unifié (ordre respecté : leçons, quizzes, assignments selon module.order) */}
                          {(() => {
                            // Construire la liste ordonnée des contenus
                            const orderedContent: Array<{ type: 'lesson' | 'quiz' | 'assignment'; data: any; index: number }> = [];
                            
                            if (module.order && Array.isArray(module.order) && module.order.length > 0) {
                              // Utiliser l'ordre défini dans module.order
                              module.order.forEach((orderItem: any) => {
                                const itemId = typeof orderItem === 'string' ? orderItem : orderItem.id;
                                const itemType = typeof orderItem === 'object' && orderItem.type ? orderItem.type : null;
                                
                                // Chercher l'item dans les différents types
                                if (itemType === 'lesson' || !itemType) {
                                  const lessonIdx = (module.content || []).findIndex(l => l.id === itemId);
                                  if (lessonIdx !== -1) {
                                    orderedContent.push({ type: 'lesson', data: module.content![lessonIdx], index: lessonIdx });
                                    return;
                                  }
                                }
                                
                                if (itemType === 'quiz' || !itemType) {
                                  const quizIdx = (module.quizzes || []).findIndex(q => q.id === itemId);
                                  if (quizIdx !== -1) {
                                    orderedContent.push({ type: 'quiz', data: module.quizzes![quizIdx], index: quizIdx });
                                    return;
                                  }
                                }
                                
                                if (itemType === 'assignment' || !itemType) {
                                  const assignmentIdx = (module.assignments || []).findIndex((a: any) => a.id === itemId);
                                  if (assignmentIdx !== -1) {
                                    orderedContent.push({ type: 'assignment', data: module.assignments![assignmentIdx], index: assignmentIdx });
                                  }
                                }
                              });
                            } else {
                              // Ordre par défaut : leçons → quizzes → assignments
                              (module.content || []).forEach((lesson, idx) => {
                              ...
                              (module.assignments || []).forEach((assignment: any, idx: number) => {
                            ...
                            return (
                              <div className="space-y-3">
                                {orderedContent.map((item, globalIdx) => {
                                  if (item.type === 'lesson') {
                                    const lesson = item.data;
                                    const lessonIndex = item.index;
                                    return (
                                      <div key={`lesson-${globalIdx}`} className="space-y-3 bg-white rounded-lg border-2 hover:border-pink-300 transition-all">
                                        <div
                                    ...
                                    const quizIndex = item.index;
                                    return (
                                      <div key={`quiz-${globalIdx}`} className="bg-white rounded-lg border-2 border-orange-200 overflow-hidden">
                                    ...
                                    return (
                                      <div key={`assignment-${globalIdx}`} className="bg-white rounded-lg border-2 border-purple-200 overflow-hidden">
                                        ...
                                        <div className="p-4">
                                          <p className="text-sm text-gray-700">{assignment.description || 'Devoir du module'}</p>
                                        </div>
                                      </div>
                                    );
                                  }
