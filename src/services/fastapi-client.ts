...
class FastAPIClient {
  ...
  /**
   * Récupérer les catégories disponibles (contexte tenant automatique)
   */
  async getCategories(): Promise<any[]> {
    return this.get<any[]>('/api/categories/');
  }

  /**
   * Créer une nouvelle catégorie (admin OF/superadmin)
   */
  async createCategory(name: string, slug?: string): Promise<any> {
    return this.post<any>('/api/categories/', { name, slug });
  }

  /**
   * Récupérer les niveaux d'un cycle
   */
  async getLevels(cycle: string): Promise<any[]> {
    return this.get<any[]>(`/api/levels/?cycle=${cycle}`);
  }

  /**
   * Attacher un média à une leçon
   */
  async attachLessonMedia(
    courseId: string, 
    moduleId: number, 
    lessonId: number, 
    params: { key?: string; url?: string }
  ): Promise<void> {
  ...
  /**
   * Attacher une ressource pédagogique à un cours
   */
  async attachCourseResource(
    courseId: string,
    resource: { name: string; key: string; size?: number }
  ): Promise<void> {
    return this.post<void>(`/api/courses/${courseId}/resources`, resource);
  }

  /**
   * Récupérer les assets médias avec filtres
   */
  async getMediaAssets(
    ...
    return this.get<any[]>(`/api/storage/assets?${params}`);
  }
  ...
  async getAssetByKey(key: string): Promise<any> {
    return this.get<any>(`/api/storage/assets/by-key?key=${encodeURIComponent(key)}`);
  }
