...
export const useAuthForm = () => {
  ...
  const { isOFContext } = useOrganization();
  ...
  const handleLogin = async (email: string, password: string) => {
    ...
    try {
      ...
      // Dans un contexte OF, on laisse le SubdomainRouter gérer la redirection
      // pour éviter les conflits de redirection
      if (isOFContext) {
        console.log('[useAuthForm] Contexte OF détecté - SubdomainRouter gérera la redirection');
        return;
      }

      // Contexte normal (non-OF) : redirection classique par rôle
