-- Création des tables pour le système OF complet (sans référence à courses)

-- Table pour les inscriptions avec signature électronique
CREATE TABLE public.inscriptions (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL,
  course_id UUID NOT NULL, -- Référence simple sans contrainte FK
...
CREATE POLICY "Instructors can view emargements without course reference" 
ON public.emargements 
FOR SELECT 
USING (
  EXISTS (
...
CREATE POLICY "Instructors can view all evaluations" 
ON public.evaluations 
FOR SELECT 
USING (
  EXISTS (
